...
LastEditTime: 2024-03-25 19:46:59
...
class DeepFM(nn.Module):
    def __init__(
        ...
        if self.nume_fea_size != 0:
            self.fm_1st_order_dense = nn.Linear(
                self.nume_fea_size, 1
            )  # 数值特征的一阶表示
        ...
        self.dense_linear = nn.Linear(
            self.nume_fea_size, self.cate_fea_size * emb_size
        )  # 数值特征的维度变换到FM输出维度一致
        self.relu = nn.ReLU()
        # for DNN
    ...
    def forward(self, X_sparse, X_dense=None):
        ...
        fm_1st_sparse_res = [
            emb(X_sparse[:, i].unsqueeze(1)).view(-1, 1)
        ...
        fm_2nd_concat_1d = torch.cat(
            fm_2nd_order_res, dim=1
        )  # [bs, n, emb_size]  n为类别型特征个数(cate_fea_size)
